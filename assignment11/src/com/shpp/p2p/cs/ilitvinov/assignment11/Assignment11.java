    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length < 2) { // when the argument includes only expression or nothing
            while (run) {
                ...
                Calculate result = new Calculate(parser.nodeList(tokens), parser.getVariables(tokens));
        ...
        } else { // when the arguments includes expression ad variables
            ...
            List<Token> tokens = Lexer.tokenize(expression);
            Parser parser = new Parser();
            new Calculate(parser.nodeList(tokens), parser.getVariables(tokens), variables).printOutResult(expression);
        }
    }
