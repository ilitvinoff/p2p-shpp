...
public class BinaryImage {
     ...
     * false - if color of image's pixel is background color
     * true - otherwise.
        ...
        if (image != null) {
            ...
            for (int x = 0; x < image.getWidth(); x++) {
                for (int y = 0; y < image.getHeight(); y++) {
                    nodes[x][y] = image.getRGB(x, y) == colorBG ? new Node(false, x, y) : new Node(true, x, y);
                }
            }
            return nodes;
        }
     ...
     */
    private static void getColorHorizontalBG(HashMap<Integer, Integer> colorMap, BufferedImage image, int borderY) {
     ...
     */
    private static void getColorVerticalBG(HashMap<Integer, Integer> colorMap, BufferedImage image, int borderX) {
